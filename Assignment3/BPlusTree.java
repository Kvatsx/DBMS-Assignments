...
public class BPlusTree<K extends Comparable<K>, T> {
    ...
    private Entry<K, Node<K, T>> deleteChildEntry(Node<K, T> parentNode, Node<K, T> node, Entry<K, Node<K, T>> entry,
            Entry<K, Node<K, T>> oldChildEntry) {
        if (!node.isLeafNode) {
            ...
            // Discarded child node case
                ...
                if (!index.isUnderflowed() || index.keys.size() == 0) {
                    ...
                    if (s > 0 && parent.children.get(s - 1) != null) {
                        splitKeyPos = handleIndexNodeUnderflow((IndexNode<K, T>) parent.children.get(s - 1), index,
                                parent);
                    } else {
                        splitKeyPos = handleIndexNodeUnderflow(index, (IndexNode<K, T>) parent.children.get(s + 1),
                                parent);
                    }
